...
public class ProductAggregate {
	...
	public ProductAggregate(CreateProductCommand createProductCommand) throws Exception{
		...
		if (createProductCommand.getPrice().compareTo(BigDecimal.ZERO) <= 0) {
			throw new IllegalArgumentException("Price cannot be negative or equal to zero.");
		}
	...
	public void handle(final ReserveProductCommand reserveProductCommand) {
		...
		if(this.quantity < reserveProductCommand.getQuantity()) {
			throw new IllegalArgumentException(String.format("Sorry, not enough stock for this product. Only %s available", this.quantity));
		}
